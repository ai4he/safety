...
% ── Title ───────────────────────────────────────────────────────────────────
\title{The Prompt Is the War: Goal Architecture as the Primary Determinant of Strategic Behavior in LLM Nuclear Simulations}
...
We present evidence that this finding is an artifact of prompt ontology rather than a property of the models themselves.
...
We find that the control condition produces high mean escalation with zero de-escalatory actions, while the operational-constraint condition produces dramatically lower escalation with the majority of actions being de-escalatory.
Permutation tests confirm statistically significant differences across conditions with very large effect sizes, and the pattern replicates within each model independently.
De-escalation options that went entirely unused under competitive framing become the dominant behavior under operational-constraint framing.
These results suggest that what the literature has reported as AI escalation tendencies is better understood as prompt-installed dimensional collapse: the reduction of a multi-axis decision space to a single evaluative dimension.
...
Our contributions are: (1)~the first controlled experiment isolating goal architecture as the independent variable in LLM nuclear escalation simulations; (2)~the first test of whether LLM escalatory behavior reflects strategic reasoning or narrative pattern-completion; (3)~introduction of an external-observer ethological framework and operational-constraint prompt design for analyzing LLM strategic behavior; (4)~application of dimensional collapse from Model-Dependent Ontology~\cite{delaflor2024introduction} as a diagnostic concept; and (5)~direct policy implications showing that AI safety in military contexts is a design problem, not a containment problem.
...
We conducted 48~games (46~completed successfully), yielding 1{,}450 turn-agent observations across all conditions, scenarios, and models.
...
While the reduction from A to D is modest in binary nuclear-use terms, the \emph{intensity} and \emph{timing} differ substantially.
A chi-square test for nuclear use rates across conditions was not significant ($\chi^2 = 0.34$, $p = .953$, $df = 3$), but this binary measure obscures the large differences in mean escalation and de-escalation behavior.
Figure~\ref{fig:nuclear} shows nuclear use rates decomposed by model.
The Qwen3~30B model shows high nuclear use across all conditions (83 to 100\%), while Llama~3.3~70B never reaches the nuclear threshold.
This model effect is large ($\eta^2 = .71$), but the \emph{direction} of the condition effect is consistent across both models.
...
In Condition~A, agents \emph{never} selected a de-escalatory option across all games and turns: 0.0\% of 154~actions.
...
The same models, facing the same strategic environment with the same options available, exhibit categorically different behavior based solely on the goal architecture of the prompt.
...
A Kruskal-Wallis test confirms that dimensional density differs significantly across conditions ($H = 33.88$, $p < .0001$), validating the dimensional collapse hypothesis: competitive framing collapses the evaluative space, leading agents to invoke fewer distinct criteria in their reasoning.
...
\begin{table}[tbp]
\centering
\caption{Pairwise comparisons: Condition~A vs.\ each experimental condition. Effect sizes computed at the game level (per-game mean escalation). Permutation $p$-values from 10{,}000 permutations. Bootstrap 95\% CIs from 10{,}000 samples.}
\label{tab:pairwise}
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Comparison} & \textbf{Perm.\ $p$} & \textbf{Cohen's $d$} & \multicolumn{2}{c}{\textbf{95\% Bootstrap CI}} \\
 & & & \textbf{Lower} & \textbf{Upper} \\
\midrule
A vs.\ B (Neutralized)             & .011   & 1.16 & 86.1  & 266.3 \\
A vs.\ C (Mutual Rationality)      & ${<}.001$ & 1.63 & 71.8  & 200.5 \\
A vs.\ D (Operational Constraint)  & ${<}.0001$ & 2.17 & 11.8  & 131.7 \\
\midrule
\multicolumn{5}{l}{\footnotesize Bootstrap CIs shown for experimental condition mean escalation.} \\
\multicolumn{5}{l}{\footnotesize Condition~A: $M = 363.0$, 95\% CI $[280.9, 448.2]$.} \\
\bottomrule
\end{tabular}
\end{table}
...
The condition main effect is highly significant: $F(3, 40) = 8.60$, $p = .0002$, $\eta^2 = .10$.
The model main effect is also significant: $F(1, 40) = 183.88$, $p < .001$, $\eta^2 = .71$, reflecting the categorical difference between Qwen3~30B (high escalation) and Llama~3.3~70B (low escalation).
The condition $\times$ model interaction is marginal: $F(3, 40) = 2.76$, $p = .055$, $\eta^2 = .03$.
Crucially, the condition effect survives when controlling for model: the A$\to$D gradient is not an artifact of model composition.

\paragraph{Per-model analysis.}
To directly test whether the condition effect is consistent within each model, we conducted separate Mann-Whitney tests on mean escalation for A~vs.~D within each model.
Both are significant: Llama~3.3~70B ($U = 36.0$, $p = .002$, $d = 8.39$) and Qwen3~30B ($U = 36.0$, $p = .002$, $d = 3.70$).
Figure~\ref{fig:per_model} shows the condition effects within each model.
While the absolute escalation levels differ substantially between models, the \emph{direction} and \emph{ordering} of the condition gradient is replicated within both models.
...
Our results provide strong support for the prompt-ontology hypothesis.
...
The mean action value dropped from 330 under competitive framing to 60 under operational-constraint framing.
...
A game-level Cohen's $d$ of 2.17 indicates that the distributions of behavior under Conditions~A and~D barely overlap, and the effect replicates within both models independently ($p = .002$ for each).
The same models, in the same strategic environment, with the same options available, produce fundamentally different patterns of behavior based solely on what the prompt tells them to optimize for.
...
The two-way ANOVA confirms this: even after accounting for the large model effect ($\eta^2 = .71$), the condition main effect remains highly significant ($F(3, 40) = 8.60$, $p = .0002$), and the condition gradient replicates within each model independently.
...
\emph{More reassuring}: LLMs may not possess inherent escalatory tendencies.
The ``AI likes nukes'' narrative may dissolve when the prompt architecture that produced it is modified.
Strategic behavior that is prompt-responsive is designable, which means it can be shaped through careful prompt engineering~\cite{reynolds2021prompt}.
The cooperative defaults observed in game-theoretic studies~\cite{akata2025playing,sun2025game} may represent the actual behavioral baseline of aligned models.
...
Sixth, the model identity effect ($\eta^2 = .71$) is large, and our design with only two models cannot fully disentangle model-level from condition-level effects; a larger model set would strengthen the variance decomposition.
However, the per-model analyses confirm that the condition gradient is significant within \emph{each} model independently ($p = .002$), and the condition $\times$ model interaction is only marginal ($p = .055$), indicating that the condition effect generalizes across models.
Seventh, our evaluation of reasoning quality through dimensional reference density relies on keyword matching across 10 semantic categories, which captures surface features of reasoning rather than its depth or quality~\cite{scherrer2023moral}.
...
Eighth, inference was conducted with temperature $T = 0.7$, introducing stochastic variation; the three runs per cell partially address this, but future work should examine sensitivity to temperature and other sampling parameters.
Ninth, the four conditions form a cumulative manipulation rather than a factorial design: each successive condition adds modifications, so observed A~vs.~D differences reflect the combined effect of removing competitive framing, adding mutual rationality, and replacing geopolitical narrative with operational language.
...
Our results demonstrate that the escalatory behavior reported across the wargaming literature is primarily an artifact of prompt design rather than an intrinsic property of the models tested.
Modifying only the goal-framing portion of the system prompt, while holding all simulation mechanics constant, produced a shift from zero de-escalation under competitive framing to majority de-escalation under operational-constraint framing, with an extremely large effect size replicated within both models independently.

Our findings extend the prompt sensitivity literature from task performance to strategic behavior in high-stakes domains, and provide empirical support for the claim that the prompt constitutes the ontological framework within which an agent reasons.
